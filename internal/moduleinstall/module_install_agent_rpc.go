...
func classifyAgentRPCInvokeError(err error) error {
	...
	if strings.Contains(message, "certificate signed by unknown authority") {
		return fmt.Errorf(
			"invoke agent run command failed: mTLS verification failed between admin and agent; agent certificate is not trusted by configured agent mTLS CA (re-bootstrap/reinstall agent to rotate cert with current agent CA): %w",
			err,
		)
	}
	if strings.Contains(message, "tls: bad certificate") {
		return fmt.Errorf(
			"invoke agent run command failed: mTLS verification failed between admin and agent (admin client certificate rejected by agent); ensure both sides use the same Aurora Agent mTLS CA: %w",
			err,
		)
	}
